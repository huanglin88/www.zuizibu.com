...
@section('script')
    <script>
        $('#J_name').keyup(function () {
            var $this = $(this);
            $$.ajax({
                url: '/api/convert-pinyin',
                data: {table: 'article_categories', 'name': $this.val()},
                success: function (data) {
                    $('#J_slug').val(data.pinyin);
                }
            });
        });
...
@section('content')
    <section class="content">
        <div class="row">
            <div class="col-md-12">
                <form method="POST" action="{{ route('admin.article-category.update', $category) }}">
                    {{ method_field('PUT') }}
                    {{ csrf_field() }}
                    <div class="box">
                        ...
                        <div class="box-body">
                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                <label>名称</label>
                                <input type="text" class="form-control" id="J_name" name="name" value="{{ $category->name }}">
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
                                <label>SLUG</label>
                                <input type="text" class="form-control" id="J_slug" name="slug" value="{{ $category->slug }}">
                                @if ($errors->has('slug'))
                                    <span class="help-block">
                            ...
                            <div class="form-group{{ $errors->has('content') ? ' has-error' : '' }}">
                                <label>内容</label>
                                <textarea id="J_content" name="content">{{ $category->content }}</textarea>
